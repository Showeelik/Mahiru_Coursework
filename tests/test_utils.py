...
def test_json_list_error(mock_logger, mock_json_load, mock_open, mock_exists):
    ...
    mock_open.assert_called_once_with('test.json', 'r', encoding='utf-8')
    mock_json_load.assert_called_once()
    mock_logger.error.assert_called_once_with('Файл test.json должен содержать список')
...
# Тест на успешный загрузку xls файла
@patch('os.path.exists', return_value=True)
@patch('pandas.read_excel')
@patch('src.utils.logger')
def test_read_xls_success(mock_logger, mock_read_excel, mock_exists):
    mock_read_excel.return_value = pd.DataFrame({'col1': [1, 2], 'col2': [3, 4]})
    result = read_data_transactions('test.xls')
    assert result == [{'col1': 1, 'col2': 3}, {'col1': 2, 'col2': 4}]
    mock_exists.assert_called_once_with('test.xls')
    mock_read_excel.assert_called_once_with('test.xls')
    mock_logger.info.assert_called_once_with('Файл test.xls успешно загружен')
